...
const weightedTip = computed(() => `预估加权工时完成度＝[Σ(普通含版本号工时×本人填写的需求进度)＋${FULL_CREDIT_TITLES.join('、')}工时]÷所属周应填工时×100%。五类工时按完整值计一次，不套用需求进度；普通无版本不计加权分子。0%有效，缺进度显示待补进度；应填为0不计算，超过100%按实际显示。${props.weightedMetric?.missingProgressHours > 0 ? `当前${number(props.weightedMetric.missingProgressHours)}h普通有版本工时待补进度，加权总工时暂不计算` : `当前加权工时 ${number(props.weightedMetric?.weightedDeliveredHours)}h`}，应填 ${number(props.metric?.standardHours)}h。`)
