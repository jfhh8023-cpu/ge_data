...
<style scoped>
.dt-delivery-card { display: flex; flex-direction: column; gap: 6px; min-width: 0; padding: 8px 10px; border: 1px solid #e8edf3; border-radius: 8px; background: var(--delivery-background); text-align: left; cursor: pointer; font: inherit; }
...
@media (max-width: 600px) {
  ...
  .dt-delivery-card :deep(.delivery-summary) { gap: 3px; grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .dt-delivery-card :deep(.delivery-metric strong) { font-size: 12px; }
  .dt-delivery-card :deep(.delivery-metric strong small) { font-size: 8px; }
}
</style>
