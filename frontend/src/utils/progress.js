...
export const WEIGHTED_PROGRESS_TIP = '普通需求新录入进度选择1%或10%至100%（每10%一档）；历史明确0%按0%，原始空进度保持为空。需求填报进度按同人、同版本、同标题累计普通版本工时与范围内最新已知进度加权，另显示真实填报覆盖。有效交付率＝有效已交付÷工作日应交付工时；加权交付率＝Σ(有效工时×进度)÷当前范围应交付工时，历史空进度仅在此计算中按100%。部门、岗位合并加权工时及应交付工时计算，个人仅算本人。已纳入人员缺填周仍计应交付，该周加权工时为0；应交付为0或日历无效时显示“—”。请假、培训、公司会议、出差、团建（含以其结尾的标题）全额计入有效交付，加权交付按其填报进度计入（默认100%，可编辑），不参与需求进度平均。'
