...
import { isFullCreditRecord, normalizeProgress, FULL_CREDIT_TITLES, FULL_CREDIT_DEFAULT_PROGRESS } from './effectiveHours.js'

export const DELIVERY_NOTE = `有效已交付＝有版本号的普通工时＋${FULL_CREDIT_TITLES.join('、')}（含以其结尾的标题）工时；五类全额计一次。有效交付率＝有效已交付÷应交付工时；加权交付率＝（普通版本工时×最新需求进度＋五类工时×最新进度）÷当前范围应交付工时，五类进度默认100%可编辑。历史空进度计算按100%，原值保留；明确0%仍按0%。部门、岗位合并加权工时及应交付工时计算，个人仅算本人；普通无版本只记录。应交付按所选周期工作日每天8小时、同人同日去重，缺填周仍计应交付且加权工时为0。应交付为0或日历无效显示“—”，有效容量下零加权工时显示0%，超过100%如实显示。`
...
export function groupRequirementProgress(records = [], capacity = {}) {
  ...
  for (const record of records) {
    if (!hasDeliveryVersion(record)) continue
    const fullCredit = isFullCreditRecord(record)
    ...
    if (!groups.has(key)) groups.set(key, { key, hours: 0, latest: record, records: [], fullCredit })
    const group = groups.get(key)
    group.hours += Number(record.hours) || 0
    group.records.push(record)
    if (isLater(record, group.latest)) group.latest = record
  }
  // REQ-071: five categories follow their latest progress; a saved null still means the default 100.
  return [...groups.values()].map(group => ({ ...group, progress: group.fullCredit ? normalizeProgress(group.latest.delivery_progress) ?? FULL_CREDIT_DEFAULT_PROGRESS : normalizeProgress(group.latest.delivery_progress) }))
}
...
export function deliveryMetricTip(metric, key) {
  ...
  const texts = {
    ...
    deliveredHours: `有效已交付＝有版本号的普通工时${f(m.versionedHours ?? (m.deliveredHours - (m.fullCreditHours || 0)))}h＋${five}（含以其结尾的标题）工时${f(m.fullCreditHours)}h＝${f(m.deliveredHours)}h。五类不论历史是否有版本均全额计一次；普通无版本不计入。`,
    ...
    weightedDeliveryRate: `加权交付率＝[Σ(普通有版本需求累计工时×范围内最新周期填写进度)＋${five}工时×最新进度]÷当前范围应交付工时×100%。当前折算${f(m.weightedDeliveredHours)}h÷应交付${f(m.standardHours)}h。${m.missingProgressHours > 0 ? `${f(m.missingProgressHours)}h历史空进度仅在本计算中按100%，原值仍为空；` : '历史空进度计算按100%；'}明确0%仍按0%。同人、同版本、同标题合并；五类按填报进度计入（默认100%，历史空值按100%）。部门、岗位合并加权工时及应交付工时计算，不平均个人百分比；个人仅算本人。${people}缺填周仍计应交付，该周加权工时为0。应交付大于0且日历有效时无加权工时显示0%；应交付为0或日历无效显示“—”；超过100%如实显示。实际进度填报覆盖${m.progressCoverage == null ? '不适用（无普通版本工时）' : `${f(m.progressCoverage)}%（已知进度${f(m.progressKnownHours)}h÷普通版本${f(m.versionedHours)}h）`}；兼容默认值不算实际填报。${calendar}`,
...
export function summarizeDelivery(records = [], capacity = null) {
  ...
  const allGroups = groupRequirementProgress(eligibleRecords, capacity)
  const groups = allGroups.filter(group => !group.fullCredit)
  const fullCreditWeightedHours = allGroups.filter(group => group.fullCredit).reduce((sum, group) => sum + group.hours * group.progress / 100, 0)
  ...
  const weightedDeliveredHours = weightedOrdinaryHours + missingProgressHours + fullCreditWeightedHours
  return {
    ...
    fullCreditHours: round(fullCreditHours), fullCreditWeightedHours: round(fullCreditWeightedHours), versionedHours: round(versionedHours),
    weightedDeliveredHours: round(weightedDeliveredHours),
    knownWeightedDeliveredHours: round(weightedOrdinaryHours + fullCreditWeightedHours),
