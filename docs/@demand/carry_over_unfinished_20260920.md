...
2026-09-20。状态：四轮设计反哺完成；用户确认 D1=所有更早周期，D2/D3 采用默认；**本地实施验收完成**，见[执行记录](../@test/carry_over_unfinished_20260920/execution.md)、[开发记录](../@development/carry_over_unfinished_20260920.md)。未部署生产。
...
- 草稿快照为整行，`_carry_over` 随草稿保存，恢复后标签仍在（实测优于设计阶段预计的“标签消失”降级）。实施中顺带修复 MariaDB 下 `draft_data` 以字符串返回导致草稿从不恢复的存量缺陷。
