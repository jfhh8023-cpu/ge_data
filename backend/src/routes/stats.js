...
router.get('/export.xlsx', async (req, res, next) => {
  try {
    ...
    const metaRows = [
      ...
      ['加权交付率', deliverySummary.missingProgressCount ? '待补进度' : deliverySummary.weightedDeliveryRate == null ? '无应填工时或基准不完整' : `${deliverySummary.weightedDeliveryRate}%`],
      ['五类有效工时', deliverySummary.fullCreditHours],
      ['加权有效已交付', deliverySummary.weightedDeliveredHours ?? '待补进度'],
    ...
    const capacityRows = deliverySummary.units.map(unit => {
      ...
      return { 人员: safeExcelText(unit.staffName), 岗位: safeExcelText(unit.role),
        ...
        有效交付率: unit.deliveryRate, 加权交付率: unit.missingProgressCount ? '待补进度' : unit.weightedDeliveryRate,
